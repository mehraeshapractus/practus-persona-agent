async function scrapeLinkedIn(linkedinUrl, apiToken, linkedinCookie) {
  const endpoint =
  ...
  // Pass li_at cookie for private profiles if available
  if (linkedinCookie) {
    input.cookie = linkedinCookie;
  }

  console.log("[Apify] Scraping:", linkedinUrl, linkedinCookie ? "(with cookie)" : "(no cookie)");
